class UserVotesController < ApplicationController
  ...
  def create
    @user_vote = UserVote.new(user_id: params[:user_id], robot_id: params[:robot_id])

    respond_to do |format|
      if @user_vote.save
        format.html { redirect_to :robots, notice: 'You casted a vote!' }
        format.json { render :robots, status: :created, location: @user_vote }
      else
        format.html { render :robots }
  ...
  private
    ...
    def user_vote_params
      params.require(:user_vote).permit(:user_id, :robot_id)
    end
end
